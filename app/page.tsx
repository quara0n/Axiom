  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <div className="lower-grid" id="activity">
        ...
        <section className="panel"><div className="panel-heading"><div><p className="eyebrow CC">Results</p><h2>Agent output</h2></div>{complete&&<span className="success-pill"><Check size={13}/> Review passed</span>}</div>{workflow.length?<div className="output-ready"><div className="output-icon"><Network size={20}/></div><h3 title={summary}>{summary||"Task in progress"}</h3><p className="workspace-path">{workspace}</p><p>{modelCalls} model calls · {repairRound} repair rounds{workers.length>0?` · ${workers.length} subagent proposals`:""}</p>{verification&&<p>Static checks only. Runtime behavior and visuals have not been tested.</p>}{files.length>0&&<ul className="generated-files">{files.map(file=><li key={file}>{file}</li>)}</ul>}<div className="workflow-list">{workflow.map((step,index)=><div className="workflow-step" key={step.agent}><span>{index+1}</span><div><strong>{step.agent}</strong><small title={step.assignment}>{step.assignment}</small>{step.result&&<pre className="agent-result">{step.result}</pre>}</div></div>)}</div>{workers.length>0&&<div className="workflow-list"><p className="eyebrow">Coder subagents</p>{workers.map(worker=><div className="workflow-step" key={worker.id}><span>↳</span><div><strong>{worker.name}</strong><small>{worker.status} · {worker.changed_files.length?worker.changed_files.join(", "):"no project changes"}</small></div></div>)}</div>}{taskId&&!running&&<button className="continue-button" onClick={()=>setContinueFrom(taskId)}>Continue from this task</button>}</div>:<div className="empty-state"><Bot size={22}/><p>Agent results and verification reports will appear here.</p></div>}</section>
      </div>
    </section>
