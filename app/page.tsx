...
export default function Home() {
  ...
  useEffect(()=>{setKey(sessionStorage.getItem("axiom-openrouter-key")||"");setModel(sessionStorage.getItem("axiom-model")||"~openai/gpt-sol-latest");return()=>timers.current.forEach(clearTimeout)},[]);
  function saveConnection(){if(!key.trim().startsWith("sk-or-")){setError("OpenRouter-nøkkelen må starte med sk-or-");return}sessionStorage.setItem("axiom-openrouter-key",key.trim());sessionStorage.setItem("axiom-model",model.trim());setError("");setSettingsOpen(false)}
  ...
  return <main className="app-shell">
    ...
    {settingsOpen&&<div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setSettingsOpen(false)}}><section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title"><button className="modal-close" aria-label="Close" onClick={()=>setSettingsOpen(false)}><X size={18}/></button><div className="connection-icon"><KeyRound size={20}/></div><p className="eyebrow">Connection</p><h2 id="settings-title">Connect OpenRouter</h2><p className="modal-copy">The key is kept in this browser session and sent only to Axiom's backend when you run a task.</p><label>API key<input type="password" value={key} onChange={e=>setKey(e.target.value)} placeholder="sk-or-v1-…" autoComplete="off"/></label><label>Orchestrator model<input value={model} onChange={e=>setModel(e.target.value)} placeholder="~openai/gpt-sol-latest"/></label>{error&&<div className="error-message">{error}</div>}<button className="connect-button" onClick={saveConnection}>Connect LLM</button></section></div>}
  </main>;
}
