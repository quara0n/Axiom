...
// interrupted run costs one click instead of a retyped prompt.
 ...
 const pipelinePercent=Math.round((doneRoles/agents.length)*100);
 ...
 const resultsPanel=<section className="panel"><div className="panel-heading"><div><p className="eyebrow CC">Results</p><h2>Agent output</h2></div>{complete&&<span className="success-pill"><Check size={13}/> Review passed</span>}</div>{workflow.length?<div className="output-ready"><div className="output-icon"><Network size={20}/></div><h3 title={summary}>{summary||"Task in progress"}</h3><p className="workspace-path">{workspace}</p><p>{modelCalls} model calls · {repairRound} repair rounds{workers.length>0?` · ${workers.length} subagent proposals`:""}</p>{verification&&<p>{verificationLabel(verification)}</p>}{files.length>0&&<ul className="generated-files">{files.map(file=><li key={file}>{file}</li>)}</ul>}{previewUrl&&<p className="workspace-path">Preview: <a href={previewUrl} target="_blank" rel="noreferrer">{previewUrl}</a></p>}<div className="workflow-list">{workflow.map((step,index)=><div className="workflow-step" key={step.agent}><span>{index+1}</span><div><strong>{step.agent}</strong><small title={step.assignment}>{step.assignment}</small>{step.result&&<pre className="agent-result">{step.result}</pre>}</div></div>)}</div>{workers.length>0&&<div className="workflow-list"><p className="eyebrow">Coder subagents</p>{workers.map(worker=><div className="workflow-step" key={worker.id}><span>↳</span><div><strong>{worker.name}</strong><small>{worker.status} · {worker.changed_files.length?worker.changed_files.join(", "):"no project changes"}</small></div></div>)}</div>}{taskId&&!running&&<><button className="continue-button" onClick={()=>void resumeTask()}>Resume this task</button><button className="continue-button" onClick={continueTask}>Continue in the prompt window</button><button className="continue-button" onClick={()=>void openPreview(taskId)}>Open the page it wrote</button></>}</div>:<div className="empty-state"><Bot size={22}/><p>Agent results and verification reports will appear here.</p></div>}</section>;
  return <main className="app-shell">
    ...
    <aside className={`sidebar ${navOpen?"open":""}`}><p className="eyebrow">Projects</p><nav aria-label="Views"><a className={`nav-item ${view==="task"?"active":""}`} href="#task" onClick={(e)=>{e.preventDefault();goTo("task");}}><Bot size={17}/> Thread</a><a className={`nav-item ${view==="activity"?"active":""}`} href="#activity" onClick={(e)=>{e.preventDefault();goTo("activity");}}><Activity size={17}/> Agent activity</a></nav><div className="sidebar-bottom">{groups.map(group=>{const open=openProjects[group.name]!==false;return <div className="project-group" key={group.name}><button className="project-row" aria-expanded={open} onClick={()=>setOpenProjects(current=>({...current,[group.name]:!open}))}>{open?<ChevronDown size={14}/>:<ChevronRight size={14}/>}<span className="project-dot violet"/><strong>{group.name}</strong><small>{group.tasks.length}</small></button>{open&&<div className="thread-list">{group.tasks.map(item=><div className={`thread-row ${item.id===taskId?"active":""}`} key={item.id}><button title={taskTitle(item)} className="thread-item" onClick={()=>void selectTask(item.id)}><span className={`thread-dot ${item.status}`}/><span className="thread-title">{taskTitle(item)}</span></button><button className="thread-delete" aria-label={`Delete ${taskTitle(item)}`} title="Delete this thread" onClick={()=>void deleteTask(item.id)}><X size={13}/></button></div>)}</div>}</div>;})}{history.length===0&&<p className="sidebar-empty">No threads yet. Write a prompt to start one.</p>}<button className="new-project" onClick={startNewTask}><Plus size={14}/> New thread</button></div></aside>
    <section className="workspace">
      <div className="page-heading"><div><p className="eyebrow">{view==="activity"?"Agent activity":project.trim()||"Axiom"}</p><h1 title={view==="activity"?"":activeTitle}>{view==="activity"?monthLabel(activityMonth):activeTitle||"New thread"}</h1></div><div className="system-status"><span/> {running?"Agents working":connected?"Backend ready":"Setup required"}{usageLine(usage)&&` · ${usageLine(usage)}`}</div></div>
      {view==="task"&&showComposer()&&!continueTarget()&&composer}
      {!showComposer()&&threadBar}
      {view==="activity"?<>{calendarPage}{activityPanel}</>:taskId?<>{teamBoard}{resultsPanel}</>:<section className="panel"><div className="empty-state"><Bot size={24}/><p>No thread open. Pick one on the left, or write a prompt above to start a new one.</p></div></section>}
      {view==="task"&&showComposer()&&continueTarget()&&composer}
    </section>
