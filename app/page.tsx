...
// interrupted run costs one click instead of a retyped prompt.
 ...
 const composer=<section className="task-card" id="task"><div className="task-label"><Sparkles size={15}/> Talk to Axiom while it&apos;s working</div><textarea id="prompt" value={task} onChange={e=>setTask(e.target.value)} placeholder={running?"Type now — it goes out when this run finishes":"What should change next?"} aria-label="Task description" rows={3}/>{pending&&<div className="queued-note">Queued: “{pending}” — sent when this run finishes.</div>}{error&&<div className="error-message">{error}</div>}<div className="task-footer"><button className="run-button" onClick={()=>void sendPrompt(task)} disabled={!task.trim()}>{running?<><Play size={15} fill="currentColor"/> Queue it</>:<><Play size={15} fill="currentColor"/> Send</>}</button>{running&&<button className="cancel-button" onClick={()=>void cancelTask()}>Cancel</button>}</div></section>;
 ...
 const teamBoard=<><div className="section-title"><div><p className="eyebrow">Team</p><h2>Agent workspace</h2></div></div><div className="pipeline-row"><div className={`pipeline${running?" live":""}`}><span style={{width:`${pipelinePercent}%`}}/></div><span className="pipeline-label">{running?`~${pipelinePercent}% · ${doneRoles}/${agents.length} roles · ${runningAgent?.name} ${runningStep?.rounds||0}/${roundBudget} rounds`:complete?`${pipelinePercent}% · ${doneRoles}/${agents.length} roles reported`:"Idle"}</span></div><div className="agent-grid">{agents.map(({name,role,icon:Icon})=>{const state=states[name];const used=workflow.find(step=>step.agent===name)?.model;const tools=agentTools(name);return <article className={`agent-card ${state}`} key={name}><div className="agent-top"><span className="agent-icon"><Icon size={18}/></span><span className={`state-dot ${state}`}/></div><h3>{name}</h3><p>{role}</p><small className="agent-model" title={used||""}>{used||""}</small><div className="agent-state">{state==="working"?"Working now":state==="done"?<><Check size={13}/> Complete</>:state==="failed"?"Failed":state==="cancelled"?"Cancelled":"Waiting"}</div>{state==="working"&&tools>0&&<small className="agent-progress">{tools} tool call{tools===1?"":"s"} so far</small>}{name==="Coder"&&workers.length>0&&<small className="agent-progress">delegated {packagesDone}/{workers.length} packages</small>}</article>})}</div></>;
